...
ship are in `strategies`. Nothing here imports the rest of the package but
...
def canonical(value: Any) -> bytes:
    ...
    A custom strategy keys on this too, so its masks agree with the built-in
    ones about which values are the same value.
    """
...
def maskingIdentity(key: str) -> str:
    ...
    recorded too, after a space: a fingerprint is hex and never holds one.
    """

    return '{} {}'.format(keyFingerprint(key), maskingImplementation())


def splitMaskingIdentity(recorded: str) -> Tuple[str, str]:
    """A recorded maskingIdentity as (fingerprint, implementation)."""
...
class Strategy:
    ...
    # module, which would make the core depend on the strategies module.
    ...
    def _buildNative(self) -> Any:
        """A native masker for this strategy and these options, or None where
        the extension isn't in use or doesn't cover the strategy. The two are
        always the same version, so one refusing options the other accepted is
        a bug, and raises.
        """
...
def policyFor(column: str, columns: Mapping[str, Mapping[str, Any]],
              defaultStrategy: Optional[Mapping[str, Any]] = None) -> Optional[Mapping[str, Any]]:
...
def changesValues(policy: Mapping[str, Any]) -> bool:
    """Whether a column under `policy` reaches the target changed: anything but
    a PASSTHROUGH strategy such as `keep`, a custom one included.
    """
