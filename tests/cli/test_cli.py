...
def test_validate_is_offline(workspace, monkeypatch):
    ...
    monkeypatch.setattr('bauta.database.Database.connect', explode)
...
def test_dry_run_connects_once_per_alias(workspace, monkeypatch):
    """Two jobs on one alias, each checked on its target and its source: it
    used to open a connection per check, each running passwordCommand again.
    """
...
def test_dry_run_still_checks_a_job_named_like_an_unreachable_alias(workspace, caplog):
    ...
    assert 'dependent: target tgt has 2 column(s)' in caplog.text
...
def test_clear_dry_run_says_when_a_table_is_not_there(workspace, capsys):
    """It used to print a plan naming tables that don't exist, and the run it
    described then failed.
    """
    connection = sqlite3.connect(str(workspace / 'demo.db'))
    connection.execute('DROP TABLE tgt')
    connection.commit()
    connection.close()

    assert main(['clear', '--quiet', '--dry-run']) == EXIT_BAD_CONFIGURATION
    assert 'would empty' not in capsys.readouterr().out
...
def test_audit_connect_flags_a_swap_of_a_table_the_target_references(schemaWorkspace, capsys):
    """A key only the source declares constrains nothing in the copy."""
    (schemaWorkspace / 'configuration' / 'jobs.yaml').write_text(AUDIT_SWAP_JOBS_YAML)
    copy = sqlite3.connect(str(schemaWorkspace / 'copy.db'))
    copy.executescript('CREATE TABLE customers (id INT PRIMARY KEY, email TEXT, tier TEXT);'
                       'CREATE TABLE orders (id INT PRIMARY KEY, customer_id INT, total INT);')
    copy.close()

    assert main(['audit', '--quiet', '--connect']) == EXIT_SUCCESS
    assert 'replaces by swap' not in capsys.readouterr().out

    copy = sqlite3.connect(str(schemaWorkspace / 'copy.db'))
    copy.executescript('DROP TABLE orders; CREATE TABLE orders (id INT PRIMARY KEY, customer_id INT REFERENCES customers(id), total INT);')
    copy.close()

    assert main(['audit', '--quiet', '--connect']) == EXIT_JOBS_DID_NOT_SUCCEED
    assert 'ERROR    loadCustomers: in copy, orders.customer_id references customers, which loadCustomers replaces by swap' \
...
def test_an_interrupted_run_exits_130(workspace, monkeypatch):
    ...
    monkeypatch.setattr('bauta.cli.run.runDataJobs', lambda **kwargs: RunResult(outcomes=[], interrupted=True))
...
def test_memory_history_and_the_manifest_can_all_live_in_tables(maskedWorkspace, capsys):
    from bauta.jobs.memory import DATABASE_MEMORY_SCHEMA
    from bauta.jobs.reporting import DATABASE_HISTORY_SCHEMA, DATABASE_MANIFEST_SCHEMA
...
def test_every_subcommand_gets_every_other_subcommands_flags_filled_in():
    """`main` fills in the flags a subcommand doesn't define, so the helpers
    several subcommands share can read them without asking first.
    """
    from bauta.cli import _buildParser, _fillSharedFlags

    parser = _buildParser()
    (subparsers,) = [action for action in parser._actions if isinstance(action, argparse._SubParsersAction)]
    defined = {action.dest for subparser in subparsers.choices.values() for action in subparser._actions} - {argparse.SUPPRESS}

    for command in ('validate', 'jobs', 'history'):
        arguments = parser.parse_args([command])
        _fillSharedFlags(parser, arguments)
        assert defined <= set(vars(arguments))


def test_discover_needs_exactly_one_way_of_choosing_tables():
    from bauta.cli import UsageError
    from bauta.cli.generate import _requireTableSelection
