"""The shipped YAML is documentation that can rot.
...
EXAMPLE_DIRECTORY = Path(__file__).resolve().parents[2] / 'example'
...
def test_the_sample_masked_jobs_share_a_domain_and_read_their_key_from_the_environment(databaseAliases):
    ...
    raw = yaml.load((CONFIGURATION_DIRECTORY / 'jobs.yaml').read_text(), Loader=yaml.FullLoader)
...
def test_every_transformer_the_sample_references_actually_resolves():
    """The sample points at bauta.transform.builtinTransforms now that those ship
