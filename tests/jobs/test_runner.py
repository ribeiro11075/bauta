...
from bauta.jobs.pipeline import PIPELINE_DEPTH, _runDataJob, _executeDataJob
...
def test_execute_data_job_streams_rather_than_materializing_the_whole_extract(monkeypatch):
    ...
    monkeypatch.setattr('bauta.jobs.pipeline.Database', _StreamingFake)
...
def _runRetryWorker(monkeypatch, tmp_path, attempt: Any, jobConfig: Any = None) -> List[Any]:
    ...
    monkeypatch.setattr('bauta.jobs.pipeline._executeDataJob', lambda job, config, databases, watermark=None: attempt())
...
def test_retry_backoff_grows_between_attempts(monkeypatch, tmp_path):
    ...
    """
    delays = []
    monkeypatch.setattr('bauta.jobs.pipeline.time.sleep', lambda seconds: delays.append(seconds))
...
def test_the_stored_watermark_is_read_again_on_each_attempt(monkeypatch):
    ...
    monkeypatch.setattr('bauta.jobs.pipeline._executeDataJob',
...
def test_a_worker_that_dies_fails_its_job_instead_of_hanging_the_run(tmp_path, sqliteDatabase):
    ...
    """
    raw = {'workers': 1, 'jobs': {
        'crashes': _sqliteJob(sqliteDatabase, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:exitAbruptly']}),
...
def test_a_dead_worker_takes_no_other_job_with_it(tmp_path, sqliteDatabase):
    ...
    result = _runJobs({
        'crashes': _sqliteJob(sqliteDatabase, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:exitAbruptly']}),
        'alongside': _sqliteJob(sqliteDatabase, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:slowly']}),
...
def test_a_job_past_its_timeout_is_stopped_and_fails(tmp_path, sqliteDatabase):
    ...
    result = _runJobs({
        'hangs': _sqliteJob(sqliteDatabase, timeoutSeconds=1, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:hang']}),
...
def test_a_changed_implementation_is_noted_but_does_not_stop_an_upsert_job(tmp_path, sqliteDatabase, monkeypatch):
    ...
    monkeypatch.setattr('bauta.jobs.keys.maskingImplementation', lambda: 'python')
    _runJobs({'masked': _maskedSqliteJob(sqliteDatabase)}, sqliteDatabase, tmp_path)

    monkeypatch.setattr(masking, 'maskingImplementation', lambda: 'bauta-rs/9.9.9')
    monkeypatch.setattr('bauta.jobs.keys.maskingImplementation', lambda: 'bauta-rs/9.9.9')
...
def test_stopping_a_job_mid_log_record_leaves_the_other_jobs_working(tmp_path, sqliteDatabase):
    ...
    """
    jobs = {'noisy': _sqliteJob(sqliteDatabase, timeoutSeconds=1, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:logWithoutPause']})}
...
def test_a_noisy_job_does_not_starve_the_others(tmp_path, sqliteDatabase):
    ...
    """
    jobs = {'noisy': _sqliteJob(sqliteDatabase, timeoutSeconds=5, sourceQueryColumnTransforms={'name': ['tests.jobs.crashingTransforms:logWithoutPause']}),
...
def test_the_pipeline_writes_chunks_in_source_order(monkeypatch, chunkSize):
    ...
    written: List[Any] = []
    monkeypatch.setattr('bauta.jobs.pipeline.Database', _pipelineFake(rows, written=written))
...
def test_a_failure_on_any_side_of_the_pipeline_surfaces(monkeypatch):
    ...
    monkeypatch.setattr('bauta.jobs.pipeline.Database', _pipelineFake(rows, failReadAt=5))
    ...
    monkeypatch.setattr('bauta.jobs.pipeline.Database', _pipelineFake(rows, failWriteAt=5))
    with pytest.raises(RuntimeError, match='the writer failed'):
        _executeDataJob('job1', _dataJobConfig(chunkSize=10), {'src': _dbConfig(), 'tgt': _dbConfig()})

    monkeypatch.setattr('bauta.jobs.pipeline.Database', _pipelineFake(rows))
...
def test_the_pipeline_leaves_no_threads_behind(monkeypatch):
    ...
    for _ in range(5):
        ...
        monkeypatch.setattr('bauta.jobs.pipeline.Database', _pipelineFake(rows, failWriteAt=3))
        with pytest.raises(RuntimeError):
            _executeDataJob('job1', _dataJobConfig(chunkSize=10), {'src': _dbConfig(), 'tgt': _dbConfig()})
...
def test_the_pipeline_follows_the_native_masker_unless_told_otherwise(monkeypatch):
    ...
    import bauta.masking.core as masking
    from bauta.jobs.pipeline import PIPELINE_DEPTH, _pipelineDepth
...
def test_a_signal_ends_the_pause_between_forever_cycles_promptly():
    """Python resumes a sleep once a signal's handler returns, so a long
    cycleSleepSeconds outlived a container's grace period and the run was
    killed mid-sleep instead of exiting on its own.
    """
    import threading
    import time

    from bauta.jobs.runner import SIGNAL_POLL_SECONDS, _sleepUnlessTerminated

    termination = {'terminating': False}
    threading.Timer(0.1, termination.__setitem__, args=('terminating', True)).start()

    started = time.monotonic()
    _sleepUnlessTerminated(60, termination)

    assert time.monotonic() - started < 0.1 + SIGNAL_POLL_SECONDS + 0.5


def test_the_pause_between_forever_cycles_lasts_as_long_as_asked_without_a_signal():
    import time

    from bauta.jobs.runner import _sleepUnlessTerminated
